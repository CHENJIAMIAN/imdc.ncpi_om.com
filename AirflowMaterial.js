
const airflowVertexShader = /* glsl */`
uniform float uTime;
uniform sampler2D uAlphaMap;
attribute vec2 uv1;

varying vec2 vUv;
varying vec2 vUv1;

void main() {
    vec3 newPosition = position;

    gl_Position = projectionMatrix * modelViewMatrix * vec4(newPosition, 1.0);

    vUv = uv;
    vUv1 = uv1;
}
`

const airflowFragmentShader = /* glsl */`
uniform float uTime;
uniform vec3 uColor;
uniform bool hasUMap;//可能并不想使用纹理，而只是想要基于颜色和 alpha 映射来创建气流效果。
uniform sampler2D uMap;
uniform sampler2D uAlphaMap;

varying vec2 vUv;
varying vec2 vUv1;

void main() {
    vec2 smokeUv = vUv;
    smokeUv.y *= 2.5;
    smokeUv.x -= uTime * 0.2;

    vec3 color = uColor;
    if (hasUMap) {
        color *= texture2D(uMap, vUv1).rgb; // Multiply color by texture color if uMap is used
    }

    float smoke = texture2D(uAlphaMap, smokeUv).r + 0.1; // Sample the alpha map for smoke effect
    smoke = smoothstep(0.2, 0.8, smoke); // Adjust smoke intensity using smoothstep

    // Apply smoke effect based on uv coordinates
    smoke *= smoothstep(0.0, 0.2, vUv1.x);
    smoke *= smoothstep(1.0, 0.8, vUv1.x);
    smoke *= smoothstep(0.0, 0.2, vUv1.y);
    smoke *= smoothstep(1.0, 0.8, vUv1.y);

    gl_FragColor = vec4(color, smoke);

    #include <tonemapping_fragment>
    #include <colorspace_fragment>
}
`

// URL of the Perlin noise texture
// const perlinNoiseTextureURL = new URL("../../assets/public/textures/perlin.png", import.meta.url).href
const perlinNoiseTextureURL = "data:image/png;base64,"

const animations = {
    add: function (animation) {
        this.list.push(animation)
    },
    removeByKey: function (key) {
        const index = this.list.findIndex(animation => animation.name === key)
        if (index > -1) {
            this.list.splice(index, 1)
        }
    },
    removeByName: function (name) {
        const index = this.list.findIndex(animation => animation.name === name)
        if (index > -1) {
            this.list.splice(index, 1)
        }
    },
    some: function (name) {
        return this.list.some(animation => animation.name === name)
    },
    find: function (name) {
        return this.list.find(animation => animation.name === name)
    },
    update: function (delta) {
        if (this.enabled) {
            this.list.forEach(animation => {
                if (animation.enabled !== false) {
                    animation.update(delta)
                }
            })
        }
    },
    list: [],
    enabled: true
}

window.animations = animations
// Uniforms for the outer airflow material
const outerAirflowUniforms = {
    uTime: { value: 0 },
    uColor: { value: new THREE.Color("#ffffff") },
    uMap: { value: null },
    hasUMap: { value: 1 }, // Indicates whether uMap is used
    uAlphaMap: { value: null }
}

// Uniforms for the inner and cold airflow material
const innerAirflowUniforms = {
    uTime: { value: 0 },
    uColor: { value: new THREE.Color("#2a82e4") },
    uMap: { value: null },
    hasUMap: { value: 0 }, // Indicates that uMap is not used initially
    uAlphaMap: { value: null }
}

// Material for outer airflow
const outerAirflowMaterial = new THREE.ShaderMaterial({
    side: 2, // DoubleSide rendering
    transparent: true,
    alphaTest: 0.1,
    uniforms: outerAirflowUniforms,
    vertexShader: airflowVertexShader,
    fragmentShader: airflowFragmentShader,
    blending: THREE.AdditiveBlending,
    depthWrite: false
})

// Material for inner and cold airflow
const innerAirflowMaterial = new THREE.ShaderMaterial({
    side: 2, // DoubleSide rendering
    transparent: true,
    alphaTest: 0.1,
    uniforms: innerAirflowUniforms,
    vertexShader: airflowVertexShader,
    fragmentShader: airflowFragmentShader,
    blending: THREE.AdditiveBlending,
    depthWrite: false
})

// Function to initialize airflow effects
function initializeAirflow () {
    // const alphaMapTexture = R.loader.loaderTexture(perlinNoiseTextureURL) // Load the alpha map texture
    const textureLoader = new THREE.TextureLoader()
    const alphaMapTexture = textureLoader.load(perlinNoiseTextureURL)
    alphaMapTexture.colorSpace = THREE.SRGBColorSpace
    alphaMapTexture.flipY = false


    alphaMapTexture.wrapS = alphaMapTexture.wrapT = THREE.RepeatWrapping // Set texture wrapping mode
    outerAirflowUniforms.uAlphaMap.value = alphaMapTexture // Assign the alpha map to the outer airflow material
    innerAirflowUniforms.uAlphaMap.value = alphaMapTexture // Assign the alpha map to the inner airflow material

    const outerAirflow = editor.scene.getObjectByName("气流") // Get the outer airflow object
    outerAirflow.castShadow = false
    outerAirflow.receiveShadow = false
    const originalOuterMaterial = outerAirflow.material // Store the original material to get the map later
    originalOuterMaterial.map.wrapS = originalOuterMaterial.map.wrapT = THREE.RepeatWrapping // Set texture wrapping mode for the original material's map
    outerAirflow.material = outerAirflowMaterial // Assign the new material to the outer airflow
    outerAirflowUniforms.uMap.value = originalOuterMaterial.map // Assign the original material's map to the new material

    const innerAirflow = editor.scene.getObjectByName("内部气流") // Get the inner airflow object
    innerAirflow.castShadow = false
    innerAirflow.receiveShadow = false
    innerAirflow.material = innerAirflowMaterial // Assign the new material to the inner airflow

    const coldAirflow = editor.scene.getObjectByName("冷气流") // Get the cold airflow object
    coldAirflow.castShadow = false
    coldAirflow.receiveShadow = false
    coldAirflow.material = innerAirflowMaterial // Assign the new material to the cold airflow

    // Tl(false) // Assuming 'Tl' is a function related to visibility or initialization, but its definition is missing

    // Add an animation to update the uTime uniform for both materials
    animations.add({
        name: "uniformsTime",
        uniforms: outerAirflowUniforms, // Target the outer airflow uniforms
        uniforms2: innerAirflowUniforms, // Target the inner airflow uniforms
        update (deltaTime) {
            this.uniforms.uTime.value -= deltaTime * 2
            this.uniforms2.uTime.value -= deltaTime * 2
        }
    })
    /*  在渲染循环中更新动画
    function renderLoop() {
        requestAnimationFrame(renderLoop);
        const delta = clock.getDelta(); // 假设你有一个 THREE.Clock 对象来获取 deltaTime
        animations.update(delta);
      // ... 其他渲染逻辑 ...
    }
    */
}


// Call the initialization function
initializeAirflow()